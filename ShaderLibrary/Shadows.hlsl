...
float3 ApplyShadowBias_CustomBias(float3 positionWS, float3 normalWS, float3 lightDirection, float shadowDepthBias, float shadowNormalBias)
{
    float invNdotL = 1.0 - saturate(dot(lightDirection, normalWS));
    float scale = invNdotL * (_ShadowBias.y + shadowNormalBias);

    // normal bias is negative since we want to apply an inset normal offset
    positionWS = lightDirection * (_ShadowBias.xxx + shadowDepthBias)+ positionWS;
    positionWS = normalWS * scale.xxx + positionWS;
    return positionWS;
}
